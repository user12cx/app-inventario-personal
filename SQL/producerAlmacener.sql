...
BEGIN
    ...
    WHERE t.usuario_id = @usuario_id
        AND tt.nombre = 'gasto'
    -- ← ahora se filtra por nombre del tipo
...
CREATE PROCEDURE [dbo].[getTransacciones]
    @usuario_id INT
-- Se elimina el valor por defecto NULL
AS
BEGIN
    SET NOCOUNT ON;
    ...
    FROM Transacciones t
        ...
        JOIN Cuentas cu ON t.cuenta_id = cu.idCuenta
    WHERE t.usuario_id = @usuario_id
    -- Se mantiene la condición del usuario
    ORDER BY t.fecha DESC;
-- Se ordena por fecha de más reciente a más antigua
END;
GO
...
BEGIN
    ...
    INSERT INTO Transacciones
        (descripcion, categoria_id, monto, tipo_id, cuenta_id, fecha, usuario_id)
    VALUES
        (@descripcion, @categoria_id, @monto, @tipo_id, @cuenta_id, @fecha, @usuario_id);
...
--     @idObjetivo      INT = NULL,
--     -- solo necesario para editar o eliminar
...
--     IF @tipo = 'agregar'
--     BEGIN
--         INSERT INTO ObjetivosAhorro
--             (nombre, monto_objetivo, monto_actual, fecha_limite, usuario_id)
--         VALUES
--             (@nombre, ISNULL(@monto_objetivo, 0), ISNULL(@monto_actual, 0), @fecha_limite, @usuario_id);

--         SELECT SCOPE_IDENTITY() AS idNuevoObjetivo;
...
CREATE PROCEDURE sp_cuentas_tipo
    ...
    -- 'agregar', 'editar', 'eliminar'
    @idCuenta     INT = NULL,
    -- necesario para editar o eliminar
    @nombre       VARCHAR(100) = NULL,
    @saldo        MONEY = NULL,
...
BEGIN
    ...
    BEGIN
        INSERT INTO Cuentas
            (nombre, saldo, estado, usuario_id)
        VALUES
            (@nombre, ISNULL(@saldo, 0), @estado, @usuario_id);

        SELECT SCOPE_IDENTITY() AS idNuevaCuenta;
...
CREATE PROCEDURE sp_usuarios_tipo
    ...
    @idUser          INT = NULL,
    -- para editar, eliminar o listar por ID
...
BEGIN
    ...
    BEGIN
        INSERT INTO Usuarios
            (usuario, name, apellidos, email, password, telefono, ocupacion)
        VALUES
            (@usuario, @name, @apellidos, @email, @password, @telefono, @ocupacion);
